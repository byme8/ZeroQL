...
public class PersistentQueryTest
{
    ...
    [Fact]
    public async Task CanPushAutomaticPersistedQueryWhenServerDoesntHaveIt()
    {
        var context = await RunServer(10_001, useAutomaticPersistedOperationPipeline: true);
        using var source = context.CancellationTokenSource;
    ...
    private static async Task<Program.ServerContext> RunServer(int port, string? queriesPath = null, bool useAutomaticPersistedOperationPipeline = false)
    {
        var context = new Program.ServerContext
        {
            ...
            Port = port,
            UseAutomaticPersistedOperationPipeline = useAutomaticPersistedOperationPipeline
        };
